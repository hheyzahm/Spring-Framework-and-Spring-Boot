...
@RestController
public class CustomerControllerS16 {
    public static final String CUSTOMER_PATH = "/api/v1/customer";
    public static final String CUSTOMER_PATH_ID = CUSTOMER_PATH + "/{customerId}";

    private final CustomerServiceS16 customerService;

    @PatchMapping(CUSTOMER_PATH_ID)
    public ResponseEntity patchCustomerById(@PathVariable("customerId") UUID customerId,
                                            @RequestBody CustomerDTOS16 customer){
    ...
    public ResponseEntity handlePost(@RequestBody CustomerDTOS16 customer){
        CustomerDTOS16 savedCustomer = customerService.saveNewCustomer(customer);
    ...
    public List<CustomerDTOS16> listAllCustomers(){
        return customerService.getAllCustomers();
    }

    @GetMapping(value = CUSTOMER_PATH_ID)
    public CustomerDTOS16 getCustomerById(@PathVariable("customerId") UUID id){

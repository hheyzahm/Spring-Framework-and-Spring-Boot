...
public class BeerServiceS15JPA implements BeerServiceS15 {
    private final BeerRepositoryS15 beerRepository;
    private final BeerMapperS15 beerMapper;

    private static final int DEFAULT_PAGE = 0;
    private static final int DEFAULT_PAGE_SIZE = 25;

    @Override
    public Page<BeerDTOS15> listBeers(String beerName, BeerStyle beerStyle, Boolean showInventory,
                                   Integer pageNumber, Integer pageSize) {

        PageRequest pageRequest = buildPageRequest(pageNumber, pageSize);

        Page<BeerEntityS15> beerPage;
    ...
    private Page<BeerEntityS15> listBeersByNameAndStyle(String beerName, BeerStyle beerStyle, Pageable pageable) {
        return beerRepository.findAllByBeerNameIsLikeIgnoreCaseAndBeerStyle("%" + beerName + "%",
                beerStyle, pageable);
    }

    public Page<BeerEntityS15> listBeersByStyle(BeerStyle beerStyle, Pageable pageable) {
        return beerRepository.findAllByBeerStyle(beerStyle, pageable);
    }

    public Page<BeerEntityS15> listBeersByName(String beerName, Pageable pageable){
        return beerRepository.findAllByBeerNameIsLikeIgnoreCase("%" + beerName + "%", pageable);
    }

    @Override
    public Optional<BeerDTOS15> getBeerById(UUID id) {
        return Optional.ofNullable(beerMapper.beerEntityToBeerDTO(beerRepository.findById(id)
                .orElse(null)));
    }

    @Override
    public BeerDTOS15 saveNewBeer(BeerDTOS15 beer) {
        return beerMapper.beerEntityToBeerDTO(beerRepository.save(beerMapper.beerDTOtoBeerEntity(beer)));
    }

    @Override
    public Optional<BeerDTOS15> updateBeerById(UUID beerId, BeerDTOS15 beer) {
        AtomicReference<Optional<BeerDTOS15>> atomicReference = new AtomicReference<>();
    ...
    @Override
    public Optional<BeerDTOS15> patchBeerById(UUID beerId, BeerDTOS15 beer) {
        AtomicReference<Optional<BeerDTOS15>> atomicReference = new AtomicReference<>();
